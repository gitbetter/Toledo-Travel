...
void LinkedList_Free(LinkedList *list) {
    ...
    for(ListNode *p = list->head; p->next != NULL; p = p->next) free(p);
    free(list);
}
...
boolean LinkedList_InsertEnd(LinkedList *list, void *item) {
    ListNode *newNode;
    if(!(newNode = (ListNode *) calloc(1, sizeof(ListNode))))
        return FALSE;
