...
boolean ArrayList_Set(ArrayList *list, size_t index, void * item) {
    if(index > list->reserved) {
        list->reserved *= (size_t) index / list->reserved + (index % list->reserved != 0 ? 1 : 0);
    }
...
boolean ArrayList_ExpandReserved(ArrayList * list, size_t reserved) {
    ...
    list->arr = (void *) realloc(list->arr, reserved * sizeof(void *));
    return TRUE;
}
...
boolean ArrayList_Compact(ArrayList * list) {
    ...
    void **new = (void **) calloc(new_reserved, sizeof(void *));
    listncpy(new, list->arr, new_reserved);
    if(!new)
        return FALSE;
    ...
    if(list->size > 0) {        // Make sure arraylist contains items before compacting
        int j = 0;
        for(int i = 0; i < list->reserved; i++)
            if(new[i])
                list->arr[j++] = new[i];
