...
boolean HashMap_Expand(HashMap *map, size_t new_size) {
    ...
    ArrayList_ExpandReserved(new->table, new_size);

    size_t old_size = map->table->reserved;
    for(int i = 0; i < old_size; i++) {
        ...
        for(ListNode *node = llist->head; node != NULL; node = node->next) {
            ...
            HashMap_Set(new, String_Get(entry->key), entry->value);
        }
    }
...
char **HashMap_Keys(HashMap *map) {
    ...
    size_t size = map->table->reserved;
    for(int i = 0; i < size; i++) {
        LinkedList *llist = (LinkedList *) ArrayList_Get(map->table, i);
        for(ListNode *node = llist->head; node != NULL; node = node->next) {
            MapEntry *entry = (MapEntry *) node->data;
			if(entry->key != NULL) {
				keys[j] = (char *) malloc(strlen(entry->key->str) + 1);
	            strcpy(keys[j++], entry->key->str);
			}
        }
    }
...
void **HashMap_Values(HashMap *map) {
    void **vals = (void **) calloc(map->size, sizeof(void *));
    int j = 0;
    ...
    for(int i = 0; i < size; i++) {
        ...
        for(ListNode *node = llist->head; node != NULL; node = node->next) {
            MapEntry *entry = (MapEntry *) node->data;
			vals[j] = (void *) malloc(sizeof(void));
            vals[j++] = entry->value;
        }
    }
...
void HashMap_Print(HashMap *map) {
    ...
    *output = '{';
    for(int i = 0; i < alist_size; i++) {
        ...
        for(ListNode *node = llist->head; node != NULL; node = node->next) {
            MapEntry *entry = (MapEntry *) node->data;
            sprintf(output + String_ChrLen(output), "[%s] = %p, ", String_Get(entry->key), entry->value);
        }
    }

    sprintf(output + String_ChrLen(output) - 2, "}\n");
